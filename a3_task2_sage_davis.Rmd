...
sw_split_name <- starwars %>%   # Start with starwars dataset
  separate(col = name,  # separate name column
            into = c("name_1", "name_2"),  # into columns name_1 and name_2
            sep = "-")  # separated at 1st column

# didn't get an error, line 49 last last part of name after 2nd dash
...
sw_sep_names <- starwars %>% # Start with starwars dataset
...
# Name column remains, dashes ignored and whole name is in "name_first" column
```
...
sw_skin_sep <- starwars %>%  #  Start with starwars dataset
  separate(col = skin_color,    # separate the 'skin_color' column)
           into = c("skin_1", "skin_2"),  # into these two columns
           sep = ",",       # split after 1st comma
           extra = "merge",   # merge data after 1st comma into 2nd column
           remove = FALSE)    #  Leave orinal column

#  skin_color column left.  A few anomalies with dashes in 1st column, and two colors ion 2nd column with 2nd column
...
mpg_make_model <- mpg %>% # Start with mpg
  unite(col = make_model, # Create a new column called 'make_model'
        ...
        sep = "^" # Puts "HEY!" between the pieces (default is underscore)
        ) 
...
mpg_model_class <- mpg %>%  # start with mpg dataset
  unite(col = model_class,  # create new column 'model_class'
        c(model, class),  # combine model and class columns
        sep = "-")        # Put '-' between values 
...
some_dates <- c(11061972, 02142007, 01301843)  # creates vector of dates w/o spaces
some_dates_converted <- mdy(some_dates)        # Stores to Date format and stores as new dataset

some_dates_converted                           # Return some_dates_converted

class(some_dates_converted)                    # Check the classvie
...
gasol_hookshots <-lakers %>%      # start with lakers dataset
  filter (player == "Pau Gasol", type == "hook") %>%    # Filter for Gasol and hook shot
  mutate(date = ymd(date))  %>%   # Update date column to proper format
  mutate(month = month(date))    # make a new column with month from date column

gasol_hookshots_month <- gasol_hookshots  %>%   # start with list of hookshots
  group_by(month, type) %>%   # group by month and type of shoot
  count(month)          # count instances per month

# View it
gasol_hookshots_month
...
storms_danger <- storms  %>%        # Start from storms data
  mutate(danger = case_when(         # add new column named danger, which contains...
    status == "hurricane" ~ "high",  # if status is hurricane, the danger is 'high'
    TRUE ~ "moderate"))               # Otherwise label is as 'moderate'
